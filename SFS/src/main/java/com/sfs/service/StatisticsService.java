...
public class StatisticsService {
     ...
     * Utilization rate = Average percentage of group members logged into devices assigned to the group.
    ...
    public Map<String, Double> getUtilizationRateLast24Hours() {
        ...
        List<Groups> groups = groupRepository.findAllWithEmployeesAndDevices();
        ...
        for (Groups group : groups) {
            ...
            for (TimeLog log : logs) {
                ...
                if (log.getEmployee().getGroups().contains(group) && group.getDevices().contains(log.getDevice())) {
                    Instant logStart = log.getStartTime();
                    Instant logEnd = log.getEndTime() != null ? log.getEndTime() : end; // Use 'end' if still logged in
